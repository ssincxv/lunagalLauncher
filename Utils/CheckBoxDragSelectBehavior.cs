...
namespace lunagalLauncher.Utils
{
    ...
    public class CheckBoxDragSelectBehavior
    {
        ...
        // ---------- ItemsRepeater 虚拟化版本 ----------
        //
        // 方案 C Phase 3 后，LlamaServicePage 的下拉列表从 ItemsControl 换成 ItemsRepeater，
        // 容器变为按需创建/销毁。我们订阅 ElementPrepared / ElementClearing 两个事件，
        // 在容器刚"准备好"时挂 behavior、在容器被回收时取消 behavior，这样虚拟化滚动时
        // 新滚入的项能正确支持拖拽多选，旧滚出的项不泄漏事件订阅。

        /// <summary>
        /// 附加行为到 <see cref="Microsoft.UI.Xaml.Controls.ItemsRepeater"/>
        /// （为每个虚拟化生成的 CheckBox 启用拖拽选择）。
        /// </summary>
        public static void Attach(Microsoft.UI.Xaml.Controls.ItemsRepeater itemsRepeater)
        {
            if (itemsRepeater == null)
                return;

            // 幂等：先 -= 再 += 防止多次 Attach（页面 Unloaded/Loaded 多次）导致重复订阅
        ...
        private static void OnRepeaterElementPrepared(Microsoft.UI.Xaml.Controls.ItemsRepeater sender,
            Microsoft.UI.Xaml.Controls.ItemsRepeaterElementPreparedEventArgs args)
        {
            var checkBox = FindCheckBoxInVisualTree(args.Element);
            if (checkBox != null)
                SetIsEnabled(checkBox, true);
        }

        private static void OnRepeaterElementClearing(Microsoft.UI.Xaml.Controls.ItemsRepeater sender,
            Microsoft.UI.Xaml.Controls.ItemsRepeaterElementClearingEventArgs args)
        {
            var checkBox = FindCheckBoxInVisualTree(args.Element);
            if (checkBox != null)
                SetIsEnabled(checkBox, false);
        }

...
namespace lunagalLauncher.Services
{
    ...
    internal static partial class MouseMappingEngine
    {
        ...
        private sealed class HookSuspension : IDisposable
        {
        ...
        internal static void Apply(MouseMappingConfig config)
        {
            ...
            if (System.Threading.Volatile.Read(ref _suspendCount) > 0)
            {
                ...
                _config = config;
                Log.Debug("鼠标映射引擎：Apply 在暂停期间仅缓存配置");
                return;
            }
            if (config.Rules != null)
                MouseMappingMigration.NormalizeActionWhenKeyComboTextPresent(config.Rules);
            ...
            while (true)
            {
                try
                {
                    ...
                    for (int b = 0; b < 5; b++)
                    {
                        ...
                        // Magpie 切换是个「有状态」动作（缩放开/关），连发语义下应避免过于密集导致来回切换；下限钳到 200ms。
                        int totalMs = rule.Action == MouseActionKind.ToggleMagpieScaling
                            ? Math.Max(200, rule.RepeatIntervalMs)
                            : Math.Max(60, rule.RepeatIntervalMs);
                        Log.Information("鼠标映射：轮询连发开始 btn={B} 规则={N} 间隔={I}ms", b, rule.Name, totalMs);

                        int clickCount = 0;
                        while (_config != null && _config.GlobalEnabled)
                        {
                            if (rule.Action == MouseActionKind.ToggleMagpieScaling)
                            {
                                MagpieController.ToggleScaling();
                            }
                            else if (rule.Action == MouseActionKind.KeyCombo)
                            {
                                InputSimulatorHelper.SendKeyCombo(rule.KeyComboText, rule.ClickMinHoldMs);
                            }
                            else
                            {
                                // 与「触发键按住时长」一致：轮询连发此前硬编码 20ms，导致设置 300ms 仍只有约 20ms。
                                if (isXButton)
                                {
                                    int holdMs = rule.ClickMinHoldMs > 0 ? rule.ClickMinHoldMs : 10;
                                    if (holdMs > 600_000) holdMs = 600_000;
                                    MouseInputNative.mouse_event(meDown, 0, 0, meData, InputSimulatorHelper.MappingSyntheticExtraInfoPtr);
                                ...
                                {
                                    InputSimulatorHelper.SendMouseButtonClickFireOnce(actionBtn, rule.ClickMinHoldMs);
                    ...
                    foreach (int kbdVk in _keyboardRepeatVks)
                    {
                        ...
                        int totalMsK = ruleK.Action == MouseActionKind.ToggleMagpieScaling
                            ? Math.Max(200, ruleK.RepeatIntervalMs)
                            : Math.Max(60, ruleK.RepeatIntervalMs);
                        Log.Information("鼠标映射：键盘轮询连发开始 vk={Vk} 规则={N} 间隔={I}ms", kbdVk, ruleK.Name, totalMsK);

                        int clickCountK = 0;
                        while (_config != null && _config.GlobalEnabled)
                        {
                            if (ruleK.Action == MouseActionKind.ToggleMagpieScaling)
                            {
                                MagpieController.ToggleScaling();
                            }
                            else if (ruleK.Action == MouseActionKind.KeyCombo)
                            {
                                InputSimulatorHelper.SendKeyCombo(ruleK.KeyComboText, ruleK.ClickMinHoldMs);
                            }
                            else
                            {
                                if (isXButtonK)
                                {
                                    int holdMsK = ruleK.ClickMinHoldMs > 0 ? ruleK.ClickMinHoldMs : 10;
                                    if (holdMsK > 600_000) holdMsK = 600_000;
                                    MouseInputNative.mouse_event(meDownK, 0, 0, meDataK, InputSimulatorHelper.MappingSyntheticExtraInfoPtr);
                                    System.Threading.Thread.Sleep(holdMsK);
                                    MouseInputNative.mouse_event(meUpK, 0, 0, meDataK, InputSimulatorHelper.MappingSyntheticExtraInfoPtr);
                                }
                                else
                                {
                                    InputSimulatorHelper.SendMouseButtonClickFireOnce(actionBtnK, ruleK.ClickMinHoldMs);
        ...
        private static void UpdatePhysicalMouseStateFromLLHook(MouseInputNative.MSLLHOOKSTRUCT info, int msg)
        {
            ...
            if (IsButtonDownMessage(msg) || IsButtonDoubleClickMessage(msg))
        ...
        private static IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            ...
            // 快速双击时系统可能只投递 WM_*DBLCLK 而无第二次 WM_*DOWN；若此时未标记按下，补一次与 DOWN 等价的 FSM。
            if (IsButtonDoubleClickMessage(msg))
            {
                if (!_btn[b].Down)
                {
                    Log.Debug("鼠标映射: HookCallback WM_*DBLCLK（补按下）btn={B} pt=({X},{Y})", b, info.pt.X, info.pt.Y);
                    return OnButtonDown(b, info, nCode, wParam, lParam);
                }
                return MouseInputNative.CallNextHookEx(_mouseHook, nCode, wParam, lParam);
            }
        ...
        /// <summary>
        /// 移动超过默认容差时是否应跳过该规则。快双击时手指微动常超过 8px，短按「单击」不应被判成拖拽而拒绝匹配；
        /// 长按/拖拽场景仍保持原逻辑（移动过远则不命中）。
        /// </summary>
        private static bool RuleSkippedDueToMoveTooFar(MouseMappingRule rule, bool movedTooFar, long durationMs)
        {
            if (!movedTooFar) return false;
            if (rule.Trigger is MouseTriggerKind.DoubleClick or MouseTriggerKind.MultiClick)
                return false;
            if (rule.Trigger == MouseTriggerKind.Click && durationMs < EffectiveClickVsHoldThresholdMs(rule))
                return false;
            return true;
        }
        ...
        /// <summary>
        /// 按住连发 + 模拟鼠标键 + 配置了「触发键按住时长」时：每次脉冲为 DOWN→保持→UP（与 FireOnce 一致）。
        /// 未配置（0）时沿用旧版 UP→DOWN 过渡连发，兼容部分游戏/引擎。
        /// </summary>
        private static bool RepeatUsesDownHoldUpPulse(MouseMappingRule rule) =>
            rule.Behavior == MouseBehaviorMode.RepeatWhileHeld
            && rule.Action == MouseActionKind.MouseButton
            && rule.ClickMinHoldMs > 0;
        ...
        private static void FireRepeatAction(int b, MouseMappingRule rule)
        {
            System.Threading.ThreadPool.QueueUserWorkItem(_ =>
            {
                try
                {
                    if (rule.Action == MouseActionKind.KeyCombo)
                        InputSimulatorHelper.SendKeyCombo(rule.KeyComboText, rule.ClickMinHoldMs);
                    else if (rule.Action == MouseActionKind.ToggleMagpieScaling)
                        MagpieController.ToggleScaling();
                    else if (RepeatUsesDownHoldUpPulse(rule))
                        InputSimulatorHelper.SendMouseButtonClickFireOnce(rule.SimulatedMouseButton, rule.ClickMinHoldMs);
                    else
                        InputSimulatorHelper.SendMouseButtonClick(rule.SimulatedMouseButton, 0);
                }
                catch (Exception ex)
                {
        ...
        private static IntPtr OnButtonUp(int b, MouseInputNative.MSLLHOOKSTRUCT info, int nCode, IntPtr wParam, IntPtr lParam)
        {
            ...
            foreach (var rule in rules)
            {
                ...
                if (RuleSkippedDueToMoveTooFar(rule, _btn[b].MovedTooFar, duration))
                    continue;
            ...
            // DOWN 已被吞时：应用从未收到物理 DOWN，不应再补发孤儿 UP。
            // - 同槽模拟鼠标键：孤儿 UP + ExecuteAction 的「DOWN→保持→UP」会被错读为两次点击。
            // - Magpie 切换：ExecuteAction 不注入任何鼠标事件，补 UP 会成为真正的孤儿 UP 投到 gal/浏览器。
            bool skipOrphanUp = downWasSuppressed
                && (matchedForBg.Action == MouseActionKind.ToggleMagpieScaling
                    || (matchedForBg.Action == MouseActionKind.MouseButton
                        && matchedForBg.SimulatedMouseButton == b));
        ...
        {
            int holdMs = Math.Max(0, rule.ClickMinHoldMs);
            switch (rule.Action)
            {
                case MouseActionKind.KeyCombo:
                    InputSimulatorHelper.SendKeyCombo(rule.KeyComboText, holdMs);
                    break;
                case MouseActionKind.ToggleMagpieScaling:
                    MagpieController.ToggleScaling();
                    break;
                default:
                    InputSimulatorHelper.SendMouseButtonClickFireOnce(rule.SimulatedMouseButton, holdMs);
                    break;
            }
        }
        ...
        private static void StartRepeatTimer(int b, MouseMappingRule rule)
        {
            ...
            int baseGap = Math.Max(1, rule.RepeatIntervalMs);
            int holdMs = Math.Max(0, rule.ClickMinHoldMs);
            // DOWN→保持→UP 模式下：计时器周期 = 连发间隔 + 按住时长，使「抬起后空闲」≈ 连发间隔（与 UI 文案一致）
            bool downHoldUp = RepeatUsesDownHoldUpPulse(rule);
            int interval = downHoldUp ? Math.Max(1, baseGap + holdMs) : baseGap;
            ...
            t.Start();
            _repeatTimers[b] = t;
            Log.Information("鼠标映射：按住连发启动 Button={B} Trigger={T} Action={A} InitialDelay={D}ms Interval={I}ms DownHoldUp={H} BaseGap={G}ms",
                rule.Button, rule.Trigger, rule.Action, initialDelay, interval, downHoldUp, baseGap);
        }
        ...
        private static bool ShouldSuppressPhysicalMouseOnDown(MouseMappingRule rule, int physicalSlotB)
        {
            ...
            if (rule.Behavior == MouseBehaviorMode.RepeatWhileHeld)
            {
                ...
                if (rule.Action == MouseActionKind.ToggleMagpieScaling) return true;
            ...
            {
                if (rule.Action == MouseActionKind.KeyCombo) return true;
                // Magpie 切换不涉及鼠标/键盘注入，但仍需吞掉物理键：否则中键物理事件会到达 gal/浏览器
                // （浏览器中键会打开链接、gal 可能弹菜单），与「按下鼠标键仅用于切 Magpie」语义不符。
                if (rule.Action == MouseActionKind.ToggleMagpieScaling) return true;
                // 无论是否同槽（如 左→左）都吞掉物理 DOWN：
                // 否则物理 DOWN 已到达应用 + 钩子拦截物理 UP + 后续合成 UP + ExecuteAction 的整段 DOWN→保持→UP
                // 会让应用侧感知为两次点击。吞掉物理 DOWN 后，应用只看到 ExecuteAction 合成的一次完整点击。
                return rule.Action == MouseActionKind.MouseButton;
            }
        ...
        /// <summary>
        /// 「屏幕边缘」判定：
        /// 1) 虚拟屏幕四边 6px 内（处理屏幕边缘触发的系统手势/捕捉区）；
        /// 2) 光标正下方窗口的非客户区（标题栏、最小化/最大化/关闭按钮、窗口边框/调整角）——
        ///    即使窗口没有顶到屏幕顶，这些区域也不应被鼠标映射吞掉，否则会让用户无法点击关闭按钮。
        /// NC 区域通过「比较顶级窗口的 GetWindowRect 与 GetClientRect 映射到屏幕坐标后的客户区」判定，
        /// 纯 Win32 只读调用，不向目标窗口 SendMessage，不会因目标进程卡死而阻塞钩子线程。
        /// </summary>
        private static bool IsOnScreenEdge(MouseInputNative.POINT pt)
        {
            ...
            const int edge = 6;
            if (pt.X <= vx + edge || pt.Y <= vy + edge || pt.X >= vx + vw - edge || pt.Y >= vy + vh - edge)
                return true;

            return IsCursorOnWindowNonClient(pt);
        }

        /// <summary>
        /// 光标是否落在其顶级窗口的非客户区（标题栏/调整框/按钮带）。<br/>
        /// 实现：取 <see cref="MouseInputNative.WindowFromPoint"/> 向上到 GA_ROOT 的顶级窗口；
        /// 对比其 GetWindowRect（屏幕坐标）与 GetClientRect（映射到屏幕坐标后的客户区）——
        /// 光标点在窗口 rect 内但不在客户 rect 内 ⇒ 非客户区。
        /// </summary>
        private static bool IsCursorOnWindowNonClient(MouseInputNative.POINT pt)
        {
            try
            {
                var h = MouseInputNative.WindowFromPoint(pt);
                ...
                if (!MouseInputNative.GetWindowRect(root, out var wrect)) return false;

                // 桌面/shell 等顶级窗口本身就是全屏，不应因此拦所有点击
                int vx = MouseInputNative.GetSystemMetrics(MouseInputNative.SM_XVIRTUALSCREEN);
                int vy = MouseInputNative.GetSystemMetrics(MouseInputNative.SM_YVIRTUALSCREEN);
                int vw = MouseInputNative.GetSystemMetrics(MouseInputNative.SM_CXVIRTUALSCREEN);
                int vh = MouseInputNative.GetSystemMetrics(MouseInputNative.SM_CYVIRTUALSCREEN);
                bool coversWholeVirtualScreen =
                    wrect.Left <= vx && wrect.Top <= vy
                    && wrect.Right >= vx + vw && wrect.Bottom >= vy + vh;
                if (coversWholeVirtualScreen) return false;

                if (pt.X < wrect.Left || pt.X >= wrect.Right || pt.Y < wrect.Top || pt.Y >= wrect.Bottom)
                    return false;

                if (!MouseInputNative.GetClientRect(root, out var crect)) return false;
                int cw = crect.Right - crect.Left;
                int ch = crect.Bottom - crect.Top;
                if (cw <= 0 || ch <= 0) return false;

                var topLeft = new MouseInputNative.POINT { X = crect.Left, Y = crect.Top };
                if (!MouseInputNative.ClientToScreen(root, ref topLeft)) return false;

                int clientL = topLeft.X;
                int clientT = topLeft.Y;
                int clientR = clientL + cw;
                int clientB = clientT + ch;

                // 在窗口矩形内但不在客户矩形内 ⇒ 非客户区（标题栏/边框/按钮带）
                return pt.X < clientL || pt.X >= clientR || pt.Y < clientT || pt.Y >= clientB;
            }
            catch
            {
                return false;
        ...
        {
            if (msg == MouseInputNative.WM_LBUTTONDOWN || msg == MouseInputNative.WM_LBUTTONUP || msg == MouseInputNative.WM_LBUTTONDBLCLK) return 0;
            if (msg == MouseInputNative.WM_RBUTTONDOWN || msg == MouseInputNative.WM_RBUTTONUP || msg == MouseInputNative.WM_RBUTTONDBLCLK) return 1;
            if (msg == MouseInputNative.WM_MBUTTONDOWN || msg == MouseInputNative.WM_MBUTTONUP || msg == MouseInputNative.WM_MBUTTONDBLCLK) return 2;
            if (msg == MouseInputNative.WM_XBUTTONDOWN || msg == MouseInputNative.WM_XBUTTONUP || msg == MouseInputNative.WM_XBUTTONDBLCLK)
            {
        ...
        private static bool IsButtonDoubleClickMessage(int msg) =>
            msg is MouseInputNative.WM_LBUTTONDBLCLK or MouseInputNative.WM_RBUTTONDBLCLK or MouseInputNative.WM_MBUTTONDBLCLK
                or MouseInputNative.WM_XBUTTONDBLCLK;

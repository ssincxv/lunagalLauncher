...
namespace lunagalLauncher.Services
{
    ...
    internal static class InputSimulatorHelper
    {
        ...
        /// 两次输入之间保持 <paramref name="holdMsBetweenUpAndDown"/> 毫秒（0 表示沿用默认约 15ms，与旧版一致）。
        /// </summary>
        internal static void SendMouseButtonClick(int button012, int holdMsBetweenUpAndDown = 0)
        {
            try
            {
                ...
                int pauseMs = holdMsBetweenUpAndDown > 0 ? Math.Min(holdMsBetweenUpAndDown, 600_000) : 15;
                if (pauseMs < 1) pauseMs = 1;
                ...
                if (!TrySendAbsoluteUpDown(upFlag, downFlag, pauseMs))
                {
        ...
        /// <summary>
        /// 「触发一次」等单次完整点击：按下→（保持 <paramref name="holdMsBetweenDownAndUp"/>）→抬起。
        ...
        internal static void SendMouseButtonClickFireOnce(int button012, int holdMsBetweenDownAndUp = 0)
        {
            try
            {
                ...
                int ms = holdMsBetweenDownAndUp > 0 ? holdMsBetweenDownAndUp : 10;
                if (ms > 600_000) ms = 600_000;
        ...
        /// </summary>
        private static bool TrySendAbsoluteUpDown(uint upFlag, uint downFlag, int pauseMsBetweenUpAndDown)
        {
            try
            {
                ...
                System.Threading.Thread.Sleep(pauseMsBetweenUpAndDown < 1 ? 1 : pauseMsBetweenUpAndDown);
        ...
        internal static void SendKeyCombo(string? keyComboText, int holdMsBetweenDownAndUp = 0)
        {
            lock (_sendKeyComboSync)
            {
                SendKeyComboCore(keyComboText, holdMsBetweenDownAndUp);
            }
        }

        private static void SendKeyComboCore(string? keyComboText, int holdMsBetweenDownAndUp = 0)
        {
            // holdMsBetweenDownAndUp>0：使用用户配置的「触发键按住时长」；否则沿用默认 ComboKeysHoldAfterDownMs。
            int holdMs = holdMsBetweenDownAndUp > 0
                ? Math.Min(holdMsBetweenDownAndUp, 600_000)
                : ComboKeysHoldAfterDownMs;
            if (string.IsNullOrWhiteSpace(keyComboText))
            {
            ...
            if (mains.Count == 1)
            {
                ...
                else
                {
                    ...
                    // 单主键无修饰键：用户配置了「触发键按住时长」时使用之，否则沿用 ComboKeyInterDownMs 的极短默认。
                    int singleHold = holdMsBetweenDownAndUp > 0 ? holdMs : ComboKeyInterDownMs;
                    System.Threading.Thread.Sleep(singleHold);

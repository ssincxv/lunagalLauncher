...
namespace lunagalLauncher.Data
{
    ...
    {
        /// <summary>
        /// 将旧版（schema 1 及以前）MouseActionKind 枚举值（None=0, Passthrough=1, MouseButton=2, KeyCombo=3, RepeatMacro=4）
        /// 迁移到 schema 2（MouseButton=0, KeyCombo=1）。
        ///
        /// 重要：schema 2 自身又扩展了 ToggleMagpieScaling=2，与旧 schema 的 MouseButton=2 数值冲突。
        /// 所以仅当 schema 还不是 "2" 时才能跑这个迁移，否则会把 ToggleMagpieScaling(2) 错误降级为 MouseButton(0)。
        /// </summary>
        public static void MigrateActionKind(List<MouseMappingRule>? rules, string? currentSchemaVersion = null)
        {
        ...
        public static void EnsureRulesFromLegacy(RootConfig config)
        {
            ...
            {
                MigrateActionKind(mm.Rules, mm.SchemaVersion);

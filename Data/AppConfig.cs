namespace lunagalLauncher.Data
{
    ...
    public class AppConfig
    {
        ...
        /// <summary>
        /// 是否最小化启动该应用（从本程序或托盘菜单单独启动、以及一键启动时与全局模式合并生效）
        /// </summary>
    ...
    public class LlamaServiceConfig
    {
        ...
        /// <summary>
        /// 用户从 GGUF 下拉列表中移除的路径（规范化后存储）。
        /// 用于避免仅从 ModelPathHistory 删除后，同一文件仍被磁盘扫描再次加入列表。
        /// Paths removed from the dropdown; prevents re-listing from scan after history removal.
        /// </summary>
        public List<string> ModelPathDropdownExcluded { get; set; } = new List<string>();

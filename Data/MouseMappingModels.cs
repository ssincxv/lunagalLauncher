namespace lunagalLauncher.Data
{
    ...
    public enum MouseActionKind
    {
        ...
        KeyCombo = 1,
        /// <summary>
        /// 切换 Magpie 全屏缩放：不注入键盘，直接向 Magpie_Hotkey 窗口 PostMessage WM_HOTKEY，
        /// 避免 Shift 等修饰键被前台 galgame 检测到触发快进。
        /// </summary>
        ToggleMagpieScaling = 2
    }
    ...
    public class MouseMappingRule
    {
        ...
        /// <summary>
        /// 模拟鼠标键时，合成「按下→保持→抬起」中保持阶段时长（毫秒）。0 表示使用引擎默认短间隔（约 10ms）。
        /// </summary>
        public int ClickMinHoldMs { get; set; } = 0;

...
namespace lunagalLauncher.Data
{
    ...
    public class ConfigPersistence
    {
        ...
        {
            // 默认 %APPDATA%\lunagalLauncher\config.json；exe 同目录存在 portable（或 portable.txt）时
            // 使用 exe\setting\config.json，并可在首次时从 Roaming 复制一份。
            _configFilePath = AppStoragePaths.GetConfigFilePath();
            AppStoragePaths.TryMigrateRoamingConfigIntoPortable(_configFilePath);
            if (AppStoragePaths.IsPortableMode)
            {
                Log.Information("便携模式：配置文件路径 {ConfigFilePath}", _configFilePath);
            }

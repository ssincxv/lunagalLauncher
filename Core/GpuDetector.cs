...
namespace lunagalLauncher.Core
{
    ...
    public class GpuDetector
    {
        ...
        // ---------- 进程级时效缓存（消除 LlamaServicePage 启动期 3 次重复扫描）----------
        //
        // 历史问题：`new GpuDetector()` 构造器会同步执行 `DetectGpus()`，
        // 而 LlamaServicePage 在同一次 Loaded 里还会走 `LoadDataAsync` + `_Loaded` 各再调一次
        // `RefreshGpuList` → `DetectGpus`，三次加起来 ~215ms 全阻塞在 UI 线程。
        // 这里统一把 nvidia-smi / WMI 扫描的结果在进程级缓存 <see cref="CacheWindow"/> 毫秒，
        // 同一时间窗内的后续构造/调用**直接复用**上次结果，不再拉起子进程和 WMI。
        //
        // 线程安全：读写缓存加锁；扫描本身不在锁里跑，避免长时间占锁。

        /// <summary>缓存有效窗口（毫秒）。用户手动点"刷新 GPU"之类动作希望强刷的场景请走 <see cref="DetectGpus(bool)"/>。</summary>
        public static int CacheWindow { get; set; } = 1000;

        private static readonly object _cacheLock = new object();
        private static List<GpuInfo>? _cachedGpus;
        private static long _cachedAtTick;

        /// <summary>
        /// 构造函数 - 自动检测 GPU（命中进程级缓存时跳过 nvidia-smi / WMI）。
        /// </summary>
        public GpuDetector()
        {
            DetectGpus();
        ...
        /// <summary>
        /// 检测系统中的所有 GPU（缓存有效则直接复用上次结果）。
        /// </summary>
        public void DetectGpus() => DetectGpus(forceRefresh: false);

        /// <summary>
        /// 检测系统中的所有 GPU。
        /// </summary>
        /// <param name="forceRefresh">
        /// true = 强制重新扫描 nvidia-smi / WMI；false = 若缓存在 <see cref="CacheWindow"/> 窗口内则直接复用。
        /// </param>
        public void DetectGpus(bool forceRefresh)
        {
            // 缓存命中路径：把上次扫描得到的列表复制到本实例的 AllGpus，避免所有调用者看到的
            // 列表被其它实例后续修改；复制成本可以忽略（典型 1-2 条）。
            if (!forceRefresh)
            {
                lock (_cacheLock)
                {
                    if (_cachedGpus != null && Environment.TickCount64 - _cachedAtTick <= CacheWindow)
                    {
                        AllGpus = new List<GpuInfo>(_cachedGpus);
                        Log.Debug("GPU 检测：命中进程级缓存（{Count} 个，{AgeMs}ms 内）",
                            AllGpus.Count, Environment.TickCount64 - _cachedAtTick);
                        return;
                    }
                }
            }
            ...
            lock (_cacheLock)
            {
                _cachedGpus = new List<GpuInfo>(AllGpus);
                _cachedAtTick = Environment.TickCount64;
            }
        }

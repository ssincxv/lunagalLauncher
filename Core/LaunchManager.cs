...
namespace lunagalLauncher.Core
{
    ...
    public enum LaunchStatus
    {
        ...
        Failed,
        /// <summary>批量启动时已在运行，跳过本次启动 / Skipped in batch because already running</summary>
    ...
    public class LaunchManager
    {
        ...
        /// <summary>
        /// 本会话已跟踪的进程在跑，或系统里已有与配置路径一致的可执行文件在跑（含启动器外手动先开的实例）。
        /// 路径检测与 <see cref="ProcessDetector.IsProcessRunning"/> 一致（应用管理页状态也用其兜底）。
        /// </summary>
        public bool IsLaunchTargetAlreadyRunning(AppConfig appConfig)
        {
            if (!string.IsNullOrEmpty(appConfig.Id) && IsApplicationRunning(appConfig.Id))
                return true;
            return ProcessDetector.IsProcessRunning(appConfig.Path ?? string.Empty);
        }
        ...
        {
            if (IsLaunchTargetAlreadyRunning(appConfig))
            {
                Log.Information("目标应用或同路径进程已在运行，跳过启动: {AppName} (ID: {AppId})", appConfig.Name, appConfig.Id);
                var skipped = new LaunchResult { AppConfig = appConfig, Status = LaunchStatus.SkippedAlreadyRunning };
                LaunchStatusChanged?.Invoke(this, skipped);
                return skipped;
            }
        ...
        public async Task<List<LaunchResult>> LaunchAllApplicationsAsync(List<AppConfig> apps, bool minimized = false)
        {
            ...
            {
                if (IsLaunchTargetAlreadyRunning(app))
                {
                    Log.Information("应用已在运行，跳过重复启动: {AppName} (ID: {AppId})", app.Name, app.Id);
                    results.Add(new LaunchResult
                    {
                        AppConfig = app,
                        Status = LaunchStatus.SkippedAlreadyRunning
                    });
                    continue;
                }

                var result = await LaunchApplicationAsync(app, minimized || app.LaunchMinimized);
                results.Add(result);
            ...
            Log.Information("批量启动完成 - 成功: {Success}, 失败: {Failed}, 跳过(已运行): {Skipped}", successCount, failedCount, skippedCount);
